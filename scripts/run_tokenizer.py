...
def main():
    ...
    parser = argparse.ArgumentParser(description="This script runs a trained SentencePiece tokenizer on all raw text files in a directory. It will generate .pt files representing examples from the raw text.")
    parser.add_argument("--tokenizer-dir", type=str, required=True, help="Path to directory containing .model and .vocab files that describe a trained tokenizer.")
    ...
    model_file, *extras = list(tokenizer_dir.glob('./*.model'))
    assert not extras, "The --tokenizer-dir argument has multiple .model files!"
    vocab_file, *extras = list(tokenizer_dir.glob('./*.vocab'))
    ...
    if not dir_to_tokenize:  # files are assumed to be in tokenizer dir
        ...
        files_to_tokenize.remove(model_file)
    ...
    tokenizer = SentencePieceProcessor(model_file=model_file.as_posix())
    pad_token = tokenizer.pad_id()  # if examples are to be packed to size --example-length, we will fill the last example with pad tokens
    ...
    eos_token = tokenizer.eos_id()  # separate packed examples with </s> token 

    # Run the SentencePiece tokenizer on the indicated files. Each line is treated as a separate example.
    tokenized_files = []
...
def tokenize_file_with_packing(tokenizer: SentencePieceProcessor, file: Path, example_length: int, pad_token: int) -> Path:
    '''
...
def tokenize_file_without_packing(tokenizer: SentencePieceProcessor, file: Path, pad_token: int) -> Path:
